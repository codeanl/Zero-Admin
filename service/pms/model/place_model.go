...
type (
	PlaceModel interface {
		...
		GetPlaceByIdOrUserID(in *pms.PlaceInfoReq) (place *Place, err error)
	}
	...
	Place struct {
		...
		Place     string `json:"place1" gorm:"type:varchar(191);comment:详细地址;not null"`   //详细地址
...
func (m *defaultPlaceModel) GetPlaceByIdOrUserID(in *pms.PlaceInfoReq) (place *Place, err error) {
	if in.Id != 0 {
...
func (m *defaultPlaceModel) GetPlaceList(in *pms.PlaceListReq) ([]*Place, int64, error) {
	...
	if in.Place != "" {
		db = db.Where("place1 LIKE ?", fmt.Sprintf("%%%s%%", in.Place))
	}
	if in.Phone != "" {
